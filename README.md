...
> **핵심 결과:** 기업 정보 공유 플렛폼 Crunchbase에서 수집한 스타트업 데이터를 Spark로 전처리·집계하여 국가·산업별 폐업률과 투자금액–생존율의 상관관계를 분석하고, **10만 개 포트폴리오 시나리오**를 몬테카를로로 탐색해 위험–수익 효율 경계와 자본 배분안을 도출했습니다.

---

## 1. 문제 정의

벤처 투자자는 한정된 자본을 여러 산업·국가의 스타트업에 배분할 때 **"폐업 가능성이 낮은 곳임과 동시에 성공(인수·IPO) 가능성은 높은 기업"** 을 판단해 투자합니다. 그러나 공개 데이터에는 펀드 수익률(IRR)이 없으므로, 본 분석은 성과를 다음으로 설정했습니다.

- **폐업률:** `status == closed` 비율 (낮을수록 안정적)
- **성공률:** `status ∈ {acquired, ipo}` 비율 (성공 엑시트)
- **생존율:** `status != closed` 비율

위의 기준을 바탕으로 국가·산업 단위의 위험을 측정하고, 이를 입력으로 몬테카를로 시뮬레이션을 실행하여 **위험 대비 기대 성공률이 가장 높은 자본 배분**를 탐색합니다.
...
>  본 버전은 Spark 병렬 분산처리 중심으로 구성했으며, NoSQL·실시간 처리는 적용 범위에서 제외했습니다.
...
| 최저 리스크 (Min Risk) | 78.65% | **21.35** | 약 20.5개 | Biotech 83.0% · PR 12.1% |
...
>
> **해석에 반드시 주의:** 이 "Biotech 집중" 결론은 자본의 80%대를 Biotech에 배분한 결과인데, 그 성공률(89%)은 §8에서 설명한 **소수 표본 아티팩트**입니다. 따라서 시뮬레이션이 가리키는 "Biotech 단독 집중 투자"는 **입력 데이터의 한계가 그대로 반영된 것**이며, 신뢰할 수 있는 단독 카테고리 값(Biotech 16.1%)으로 재산출하면 결론이 달라질 수 있습니다. 발표 시 이 점을 명시해야 합니다.
